...
R script: [github](https://github.com/BSC6926-B52-Fall2023/workshopScripts/blob/main/workshop3.R)
      
[R script of workshop 2](workshopScripts/workshop3.R)
...
#Check the properties in Environment - vector, table, list?
models <- mtcars %>% 
  split(.$cyl) %>%
  map(~lm(mpg ~ wt, data = .x))

models <- mtcars |>  # note error in map need to figure out why 
  split(.$cyl) |> 
  map(~lm(mpg ~ wt, data = .x))
...
[Exercise solutions](ExSolutions/Key_ws3.html)